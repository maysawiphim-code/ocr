...
ขั้นตอน:
1. เปรียบเทียบกับสินค้าในร้าน CJ MORE / CJ Express ไทย
2. ถ้าไม่พบ ให้เปรียบเทียบกับสินค้าสะดวกซื้อไทยทั่วไป (7-Eleven, Lotus's Express)
3. แก้ชื่อที่ OCR อ่านผิด เช่น:
   - "ยูโร ติก สสสตางค" → "ยูโรติก สติ๊กสตางค์"
   - "มาม่า หมุน" → "มาม่า หมูสับ"
   - "โอโนะ" → "โอโชะ"
   - ตัวอักษรเพี้ยน เช่น ม↔น, ว↔ง, สระผิด
4. จัดหมวดหมู่จาก 8 หมวด:
   Bao Cafe / อาหารพร้อมทานและเบเกอรี่ / ขนมและของขบเคี้ยว / เครื่องดื่ม /
   ของใช้ส่วนตัว / ของใช้ในบ้าน / เวชภัณฑ์และอุปกรณ์ดูแลสุขภาพ / สินค้าเบ็ดเตล็ดอื่นๆ

ตอบ JSON เท่านั้น:
{{"name": "ชื่อที่ถูกต้อง", "category": "หมวดหมู่", "confidence": "high หรือ low"}}"""
...
2. ถ้าหาไม่พบใน CJ MORE ให้เปรียบเทียบกับสินค้าที่ขายทั่วไปในร้านสะดวกซื้อไทย (7-Eleven, Lotus's Express, Family Mart)
...
# ── วางระดับ module ใต้ CATEGORY_PROMPT ──────────────────────────────────────
...
- มาม่า/บะหมี่กึ่งสำเร็จรูป (มาม่า, ไวไว, ยำยำ)
- ยูโรเค้ก, เค้กทุกชนิด, ขนมปัง, แซนด์วิช, ยูโรติก (ขนมปังกรอบ/เวเฟอร์)
...
def _is_bao_item(name: str) -> bool:
    ...
    return bool(re.search(
        r'\bb[a8e๐o0][o๐0][\W_]|'   # Bao/Beo/B80/Be0 ตามด้วยช่องว่างหรือ _
        r'\bb[a8e๐o0][o๐0]$|'
        r'\bbo[\W_]|'         # ท้ายบรรทัด
        r'bao|'                        # ปกติ
...
- บางครั้งสินค้า 2 รายการถูก OCR รวมเป็นบรรทัดเดียว เช่น "1 เอสเอ็มเอส แดง 1 โอโนะข้าวอบกรอบรสค"
  → ให้แยกเป็น 2 รายการ: "1 เอสเอ็มเอส แดง" และ "1 โอโนะข้าวอบกรอบรสค"
  → สังเกตจากตัวเลขจำนวน (1, 2, 3...) ที่อยู่กลางบรรทัด
- ราคาอาจอยู่ห่างจากชื่อสินค้ามาก ให้จับคู่ตามลำดับ: สินค้าแรก → ราคาชุดแรก, สินค้าที่สอง → ราคาชุดที่สอง"""
    else:
...
- บางครั้ง OCR อ่านชื่อ Bao ผิดเป็น "Beo", "B80", "Be0", "Bo" — ให้แปลงเป็น "Bao" เสมอ
- บางครั้งสินค้า 2 รายการถูก OCR รวมเป็นบรรทัดเดียว เช่น "1 สินค้า ก 1 สินค้า ข"
  → ให้แยกเป็น 2 รายการโดยสังเกตจากตัวเลขจำนวนที่อยู่กลางบรรทัด"""
